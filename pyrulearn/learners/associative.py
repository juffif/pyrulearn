...
(`pyrulearn.learners.cba`), `CMAR` (`pyrulearn.learners.cmar`), and `IDS`
...
doesn't apply to it -- see each module's own docstring.
...
from ..models import (
    ConceptModel, FlatRuleSet, MajorityClass, PooledRuleSet, RuleView, SingleRule, annotate_default_rule,
)
...
def coverage_select(
    ...
    *after* keeping `kept[:i+1]` -- exactly what `CBA` needs to find its
    own minimum-error prefix (see `pyrulearn.learners.cba`). A caller
    that pools every surviving rule for voting instead of committing to
    one first-match prefix (`pyrulearn.learners.cmar.CMAR`) can simply
